...
final class WordServiceTests: XCTestCase {
    ...
    func testDetectLanguage() {
        ...
        XCTAssertEqual(wordService.detectLanguage(from: "你好世界"), .Chinese)
        ...
        // Test mixed content
        XCTAssertEqual(wordService.detectLanguage(from: "Hello 你好"), .Chinese)  // Should detect Chinese due to priority
    ...
    func testTextToSentences() {
        ...
        let chineseSentences = wordService.textToSentences(text: chineseText, language: .Chinese)
        XCTAssertEqual(
            chineseSentences,
            [
                "你好世界。",
        ...
        let mixedChineseSentences = wordService.textToSentences(
            text: mixedChineseText, language: .Chinese)
    ...
    func testTextToWords() {
        ...
        let chineseWords = wordService.textToWords(text: chineseText, language: .Chinese)
        XCTAssertEqual(chineseWords, ["你", "好", "世", "界", "。", "这", "是", "一", "个", "测", "试", "！"])

        // Test mixed text
        let mixedText = "Hello 你好 world!"
        let mixedChineseWords = wordService.textToWords(text: mixedText, language: .Chinese)
        XCTAssertEqual(
            mixedChineseWords,
    ...
    func testSimilarityScoreBasic() async throws {
        // Test empty strings
        let emptyScore = try await wordService.similarScore(
            "", "", language: .English, use_embedding: false)
        XCTAssertEqual(emptyScore, 0.0)

        // Test identical strings
        let identicalScore = try await wordService.similarScore(
            "test", "test", language: .English, use_embedding: false)
        XCTAssertEqual(identicalScore, 1.0)

        // Test simple English strings
        let englishScore = try await wordService.similarScore(
            "hello world", "hello earth", language: .English, use_embedding: false)
        XCTAssertGreaterThanOrEqual(englishScore, 0.5)

        // Test simple Chinese strings
        let chineseScore = try await wordService.similarScore(
            "你好世界", "你好地球", language: .Chinese, use_embedding: false)
        XCTAssertGreaterThanOrEqual(chineseScore, 0.5)

        // Test mixed language strings
        let mixedScore = try await wordService.similarScore(
            "Hello 世界", "Hello 地球", language: .Chinese, use_embedding: true)
    ...
    func testSimilarityScoreWithEmbedding() async throws {
        ...
        for (text1, text2) in englishSentences {
            let score = try await wordService.similarScore(
                text1, text2, language: .English, use_embedding: true)
        ...
        var s1 = try await wordService.similarScore(
            "在过去两年里", "过去两年的时间里", language: .Chinese, use_embedding: true)
        var s2 = try await wordService.similarScore(
            "在过去两年里", "在过去的七八年里", language: .Chinese, use_embedding: true)
        XCTAssertGreaterThanOrEqual(s1, s2)
            

        for (text1, text2) in chineseSentences {
            let score = try await wordService.similarScore(
                text1, text2, language: .Chinese, use_embedding: true)
            XCTAssertGreaterThanOrEqual(score, 0.0, "Similarity score should be >= 0")
            XCTAssertLessThanOrEqual(score, 1.0, "Similarity score should be <= 1")
    ...
    func testSimilarityScoreWithoutEmbedding() async throws {
        ...
        for (text1, text2) in englishSentences {
            let score = try await wordService.similarScore(
                text1, text2, language: .English, use_embedding: false)
            XCTAssertGreaterThanOrEqual(score, 0.0, "Similarity score should be >= 0")
            XCTAssertLessThanOrEqual(score, 1.0, "Similarity score should be <= 1")
        }
        ...
        for (text1, text2) in chineseSentences {
            let score = try await wordService.similarScore(
                text1, text2, language: .Chinese, use_embedding: false)
            XCTAssertGreaterThanOrEqual(score, 0.0, "Similarity score should be >= 0")
            XCTAssertLessThanOrEqual(score, 1.0, "Similarity score should be <= 1")
        }
    }
